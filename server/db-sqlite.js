...
const databasePath = process.env.DATABASE_PATH || (process.env.VERCEL ? path.join(os.tmpdir(), 'recovery.sqlite') : path.join(__dirname, 'recovery.sqlite'));
...
const statements = {
  ...
  metrics: db.prepare("SELECT (SELECT COALESCE(SUM(amount), 0) FROM invoices WHERE status = 'open') AS amount_at_risk, (SELECT COALESCE(SUM(amount), 0) FROM invoices WHERE status = 'paid') AS amount_recovered, (SELECT COUNT(*) FROM audit_log WHERE action IN ('escalate_to_manager', 'escalate_to_human')) AS escalation_count, (SELECT COUNT(*) FROM promises) AS promise_total, (SELECT COALESCE(SUM(status = 'broken'), 0) FROM promises) AS broken_promises")
};
...
module.exports = { db, statements, reset, ready: Promise.resolve(), isNeon: false };
