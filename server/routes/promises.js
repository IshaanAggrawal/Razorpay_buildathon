...
router.post('/', async (req, res, next) => {
  try {
    ...
    if (!parsed.date) {
      statements.insertAudit.run(invoice.id, 'system', 'escalate_to_manager', `Customer reply "${req.body.raw_text.trim()}" contains no actionable payment date; no promise was created.`);
